...
describe('ethereum-mock helper', () => {
  ...
  it('installs window.ethereum and answers eth_chainId', async () => {
    ...
    const result = await (window as Window & { ethereum?: { request: (arg: { method: string }) => Promise<string> } }).ethereum?.request({ method: 'eth_chainId' })
  ...
  it('restoreEthereum removes window.ethereum', () => {
    mockEthereum()
    restoreEthereum()
    expect((window as Window & { ethereum?: unknown }).ethereum).toBeUndefined()
  })
