...
(`msg.sender`) of the contract that calls `Nox.fromExternal()`. The
`handleProof` is bound to that caller at encryption time. If the call is routed
through an intermediary contract, the on-chain `msg.sender` no longer matches
the proof and the transaction reverts with `InvalidProof`. Trustless
multi-contract routing of an externally-encrypted handle is not currently
supported; the input must be re-encrypted for each target contract that calls
`fromExternal()`.
