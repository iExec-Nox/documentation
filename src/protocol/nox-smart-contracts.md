...
off-chain and stores it in the Handle Gateway before it can be used as an operand.
...
For user inputs, the prehandle is a random value generated by the Handle Gateway.
The handle is then validated on-chain via an EIP-712 signed `HandleProof`.
